...
impl SciVal {
    ...
    pub fn fun_comp(self, other: SciVal) -> EvalResult<SciVal> {
        match (self, other) {
            (VCallable(f1), VCallable(f2)) => Ok(VCallable(f1.fun_comp(f2)?)),
            (v1, VCallable(f2)) => f2.fun_app(vec![Arg::Val(Box::new(v1))]),
    ...
    pub fn list_slice(self, slice: Slice<i32, Option<i32>>) -> EvalResult<SciVal> {
       match self {
            ...
            VCallable(f) => {
                Ok(VCallable(f.fun_comp(ListSlice(slice, None))?))
            }
            _ => Err(EvalError::TypeMismatch)
        }
    }
        ...
        match self {
            Matrix(r, c, v) => {
                ...
                if let (Slice::Single(i), Slice::Single(j)) = (&slice1, &slice2) {
                    return Ok(VNumber(v[(*i)*r + (*j)]));
                }
                let (r, c, v) = matrix::matrix_slice(r, c, &v, slice1, slice2);
                Ok(Matrix(r, c, v))
            }
            VCallable(f) => {
                Ok(VCallable(f.fun_comp(MatrixSlice(slice1, slice2, None))?))
            }
             _ => Err(EvalError::TypeMismatch)
         }
    }
    ...
    pub fn pow(self, rhs: SciVal) -> EvalResult<SciVal> {
        match (self, rhs) {
            (VNumber(n1), VNumber(n2)) => Ok(VNumber(n1.pow(n2)?)),
            _ => Err(EvalError::TypeMismatch),
        }
    }
    ...
    pub fn conjugate(self) -> EvalResult<SciVal> {
        match self {
            VNumber(n) => Ok(VNumber(n.conjugate())),
            Matrix(r, c, v) => {
    ...
    pub fn equals(&self, rhs: &SciVal) -> EvalResult<SciVal> {
        match (self, rhs) {
            ...
            (VNumber(n1), VNumber(n2)) => Ok(Bool(n1 == n2)),
    ...
    pub fn compare(&self, rhs: &SciVal) -> EvalResult<cmp::Ordering> {
        if let (VNumber(n1), VNumber(n2)) = (self, rhs) { n1.compare(n2) }
        else { Err(EvalError::TypeMismatch) }
    }
...
impl ops::Add<SciVal> for SciVal {
    ...
    fn add(self, rhs: SciVal) -> EvalResult<SciVal> {
        match (self, rhs) {
            ...
            (VNumber(n), Matrix(r, c, m)) |
            (Matrix(r, c, m), VNumber(n)) => {
            ...
            (VNumber(n1), VNumber(n2)) => Ok(VNumber(n1 + n2)),
...
impl ops::Rem for SciVal {
   ...
   fn rem(self, rhs: SciVal) -> EvalResult<SciVal> {
       if let (VNumber(n1), VNumber(n2)) = (self, rhs) {
           Ok(VNumber((n1 % n2)?))
       } else { Err(EvalError::TypeMismatch) }
   }
}
